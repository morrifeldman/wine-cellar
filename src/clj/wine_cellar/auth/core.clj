    ...
    (when oauth-client
      (let [state (str (UUID/randomUUID))]
        ...
        (str (:authorization-uri oauth-client)
             ...
             "&redirect_uri=" (java.net.URLEncoder/encode (:redirect-uri oauth-client) "UTF-8")
             "&response_type=code"
             "&scope=" (java.net.URLEncoder/encode (str/join " " (:scope oauth-client)) "UTF-8")
             "&state=" state)))))

(defn redirect-to-google [request]
...
(defn exchange-code-for-token [code]
  ...
  (let [oauth-client (create-oauth-client)]
    (when oauth-client
      (try
        (let [request-params {:form-params {:code code
                                            :client_id (:client-id oauth-client)
              ...
              {:keys [status body error]} @(http/post (:token-uri oauth-client) request-params)]
          (tap> ["token-response" {:status status :error error :body body}])
          (if (or error (not= status 200))
            (do
              (println "Error exchanging code for token:" (or error (str "HTTP " status)))
              nil)
...
(defn get-user-info [access-token]
  ...
  (try
    (let [{:keys [status body error]} @(http/get
      ...
      (if (or error (not= status 200))
        (do
          (println "Error getting user info:" (or error (str "HTTP " status)))
          nil)
        (json/read-value body)))
    (catch Exception e
...
(defn handle-google-callback [request]
  (tap> ["handle-google-callback" (:query-params request) "session" (:session request)])
  (let [code (get-in request [:query-params "code"])
    ...
    (if error
      (do
        (println "OAuth error from Google:" error)
  ...
  (fn [request]
    ...
    (if (authenticated? request)
      ...
      (if (str/starts-with? (:uri request) "/api")
        ...
        (let [json-body (json/write-value-as-string {:error "Authentication required"})
