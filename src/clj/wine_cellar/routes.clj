...
(s/def ::wine_variety (s/keys :req-un [::variety_id]
                              :opt-un [::percentage]))
(s/def ::wine_varieties (s/coll-of ::wine_variety))

(def grape-variety-schema
  (s/keys :req-un [::variety_name]))
     ...
     ["/:id/varieties"
      {:parameters {:path {:id int?}}
       ...
       :post {:summary "Add grape variety to wine"
              ...
              :responses {201 {:body map?} 400 {:body map?} 404 {:body map?} 500 {:body map?}}
     ...
     ["/:id/varieties/:variety-id"
      {:parameters {:path {:id int? :variety-id int?}}
       :put {:summary "Update grape variety percentage for wine"
             ...
             :responses {200 {:body map?} 400 {:body map?} 404 {:body map?} 500 {:body map?}}
