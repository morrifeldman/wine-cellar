...
(def sample-wines
  [{:wine {:producer "Clos des Papes"
           ...
           "Brimming with garrigue and spice; already approachable."}
    ...
    [{:tasting_date "2024-03-18"
     ...
     {:tasting_date "2025-07-18"
      :rating 96
      :notes
      "WSET grid practice note highlighting the wine's layered Grenache-led profile."
      :wset_data
      {:note_type "wset_level_3"
       ...
       :nose {:condition "CLEAN"
              ...
              {:primary {:red-fruit ["RED CHERRY" "RED PLUM"]
       ...
       :palate {:sweetness "DRY"
                ...
                {:primary {:red-fruit ["RED PLUM" "RED CHERRY"]
                           :black-fruit ["BLACKBERRY"]
                           :herbal ["LAVENDER"]}
                 :secondary {:oak ["CEDAR" "SMOKE"]}
                 :tertiary {:bottle-age ["LEATHER" "FOREST FLOOR"]}}
                :other_observations
                "Supple tannins with a warm garrigue finish."}
       :conclusions {:quality-level "OUTSTANDING"
                     :readiness "DRINK OR HOLD"
                     :final_comments
                     "Ideal with lamb roasted with Provençal herbs."}}}]}
   {:wine {:producer "Ridge"
           :name "Monte Bello"
           :vintage 2015
           ...
           :purveyor "Winestone"
           :tasting_window_commentary
           "Decant for layered cassis, cedar, and graphite."}
   ...
   {:wine {:producer "Cloudy Bay"
           ...
           :purveyor "Total Wine"
           :tasting_window_commentary
           "Vibrant and zesty; best slightly chilled."}
    :varieties [{:name "Sauvignon Blanc" :percentage 100.0M}]
    :tasting-notes
    [{:tasting_date "2024-04-02"
      :rating 92
      :notes "Passionfruit, lime zest and a crunchy saline finish."}
     {:tasting_date "2025-07-12"
      :rating 93
      :notes
      "Structured WSET tasting grid note capturing the wine's citrus-herbal drive."
      :wset_data
      {:note_type "wset_level_3"
       :version "1.0"
       ...
       :nose {:condition "CLEAN"
              ...
              :aroma-characteristics
              {:primary {:citrus-fruit ["GRAPEFRUIT" "LEMON"]
       ...
       :palate {:sweetness "DRY"
                ...
                :finish "MEDIUM+"
                :flavor-characteristics
                {:primary {:citrus-fruit ["GRAPEFRUIT" "LIME (JUICE OR ZEST?)"]
                           :tropical-fruit ["PASSION FRUIT"]
       ...
       :conclusions {:quality-level "VERY GOOD"
                     :readiness "DRINK OR HOLD"
                     :final_comments
                     "Perfect with goat cheese and citrus-dressed greens."}}}]}
   ...
   {:wine {:producer "Jacquesson"
    ...
    :tasting-notes
    [{:tasting_date "2024-05-18"
      :rating 93
      :notes
      "Disgorged 2023-05-12; laser-cut citrus, toasted almond and saline finish."}]}
   {:wine {:producer "Penfolds"
           ...
           :purveyor "Benchmark"
           :tasting_window_commentary
           "Plush dark fruit with savory spice; share at BBQs."}
   ...
   {:wine {:producer "Lapostolle"
           ...
           :purveyor "Flatiron"
           :tasting_window_commentary
           "Luxurious Carmenère-led blend with velvety tannins."}
   ...
   {:wine {:producer "Dr. Loosen"
           ...
           :purveyor "Skurnik"
           :tasting_window_commentary
           "Ginger, white peach and slate; off-dry and electric."}
    :varieties [{:name "Riesling" :percentage 100.0M}]
    :tasting-notes [{:tasting_date "2024-04-20"
                     :rating 91
                     :notes
                     "Peach, lime and honeycomb balanced by Mosel lift."}]}
   ...
   {:wine {:producer "Château d'Yquem"
           ...
           :tasting_window_commentary
           "Botrytis-laden apricot, saffron and crème brûlée."}
   ...
   {:wine {:producer "La Vieille Ferme"
           ...
           :tasting_window_commentary
           "Easy citrus-and-stone-fruit weeknight white."}
    ...
    :tasting-notes [{:tasting_date "2024-03-01"
                     :rating 88
                     :notes
                     "Crisp lemon, white flowers and a hint of almond."}]}
   {:wine {:producer "Dominus Estate"
           ...
           :purveyor "Benchmark"
           :tasting_window_commentary
           "Powerful but tightly wound; cellar for true fireworks."}
    :varieties [{:name "Cabernet Sauvignon" :percentage 85.0M}
                {:name "Cabernet Franc" :percentage 10.0M}
                {:name "Petit Verdot" :percentage 5.0M}]
    :tasting-notes
    [{:tasting_date "2024-06-10"
      :rating 97
      :notes
      "Inky cassis, pencil shavings and crushed rock—firm structure, needs years."}]}
   {:wine
   ...
   {:wine
    {:producer "Weingut Keller"
     ...
     :purveyor "Lyle Fass"
     :tasting_window_commentary
     "Slate, citrus oil and white tea—holds tight now, patience rewarded."}
    :varieties [{:name "Riesling" :percentage 100.0M}]
    :tasting-notes
    [{:tasting_date "2024-11-18"
      :rating 95
      :notes
      "Laser-focused lime, crushed stone and hint of smoke; currently coiled."}]}
   {:wine {:producer "Scar of the Sea"
           ...
           :tasting_window_commentary
           "Saline, citrus-driven chardonnay; beautifully transparent."}
    :varieties []
    :tasting-notes []}
   {:wine {:producer "Ar.Pe.Pe."
           ...
           :purveyor "Flatiron"
           :tasting_window_commentary
           "Mountain Nebbiolo with rose petals and alpine herbs."}
    :varieties [{:name "Nebbiolo" :percentage 100.0M}]
    :tasting-notes
    [{:tasting_date "2024-07-08"
      :rating 92
   ...
   {:wine {:producer "Domaines Ott"
           ...
           :purveyor "K&L Wines"
           :tasting_window_commentary
           "Citrus peel, peach and sea spray; chill and pour on warm evenings."}
    :varieties [{:name "Grenache" :percentage 50.0M}
                {:name "Cinsault" :percentage 30.0M}
                {:name "Syrah" :percentage 20.0M}]
    :tasting-notes
    [{:tasting_date "2024-06-21"
      :rating 90
      :notes
      "Wild strawberry, grapefruit zest and a saline snap keep it refreshing."}]}
   {:wine {:producer "Cleto Chiarli"
    ...
    :tasting-notes
    [{:tasting_date "2024-09-18"
      :rating 91
      :notes
      "Black cherry, violet and a hint of balsamic—peppery sparkle keeps it lifted."}]}])

(defn find-wine-id
  [{:keys [producer name vintage]}]
  (let [vintage-clause
        (if (some? vintage) [:= :vintage vintage] [:is :vintage nil])
        where [:and [:= :producer producer] [:= :name name] vintage-clause]]
    (some-> (jdbc/execute-one! ds
                               (sql/format
                                {:select [:id] :from :wines :where where})
                               db-opts)
            :id)))

(defn ensure-variety!
  [name]
  (if-let [existing (jdbc/execute-one! ds
...
(defn seed!
  ...
  (let [ids (map upsert-wine! sample-wines)
        wines (jdbc/execute! ds
                             (sql/format {:select [:id :producer :name :vintage
