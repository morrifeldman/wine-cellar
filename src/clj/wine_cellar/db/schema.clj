...
(def classifications-table-schema
  {:create-table [:wine_classifications :if-not-exists],
   :with-columns [[:id :serial :primary-key] [:country :varchar [:not nil]]
                  [:region :varchar [:not nil]] [:aoc :varchar]
                  [:communal_aoc :varchar] [:classification :varchar]
                  [:vineyard :varchar] [:levels :wine_level :array]
...
(def wines-table-schema
  {:create-table [:wines :if-not-exists],
   :with-columns [[:id :serial :primary-key] [:producer :varchar]
                  ...
                  [:drink_until_year :integer] [:label_image :bytea]
