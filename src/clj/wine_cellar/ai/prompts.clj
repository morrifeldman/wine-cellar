...
(defn drinking-window-system-prompt
  ...
  (str
   ...
   "Focus on when this wine is or was at its absolute peak quality and most enjoyable, "
   "not just when it's acceptable to drink. Based on wine characteristics "
   "including tasting notes and grape varieties, identify the timeframe when "
   "this wine expresses its best characteristics.\n\n"
   "PURPOSE: This window is a practical tool for prioritizing what to drink from a real cellar, "
   "not an aspirational best-case ideal. It must reflect the wine's ACTUAL condition based on the "
   "evidence provided — especially the tasting notes — even when that condition is unflattering. "
   "The window is NOT bounded by the current year on either end.\n\n"
   "WINES PAST THEIR PEAK: If the evidence indicates the wine has already reached or passed its peak "
   "(e.g., tasting notes describing it as tired, faded, or past its best; an entry-level wine in a "
   "fast-aging format such as a half-bottle; or a vintage old enough to have matured), say so honestly:\n"
   "- Set drink_from_year to the year the optimal window actually opened, even if that year is in the past. "
   "Do NOT shift it forward to the current year to avoid a past date.\n"
   "- If the wine is in clear decline, set drink_until_year at or before the current year so it is correctly "
   "flagged as past its prime, rather than politely placing the window in the future.\n"
   "- Your numbers MUST agree with your reasoning: never write that peak has passed while returning a window "
   "that opens at the current year or later.\n\n"
   ...
   "- drink_from_year: (integer) The year the optimal drinking window opens — when the wine first reaches peak quality. May be a past year for already-mature wines.\n"
   "- drink_until_year: (integer) The last year the wine stays at peak quality (not merely drinkable). May be at or before the current year for wines already in decline.\n"
