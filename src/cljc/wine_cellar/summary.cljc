...
(def ^:private vintage-bands
  [{:label "\u22642000" :max 2000} {:label "2001-2010" :min 2001 :max 2010}
   {:label "2011-2018" :min 2011 :max 2018} {:label "2019+" :min 2019}])

(def ^:private price-bands
  [{:label "Under $25" :max 25} {:label "$25 - $49" :min 25 :max 50}
   {:label "$50 - $99" :min 50 :max 100}
   {:label "$100 - $199" :min 100 :max 200} {:label "$200+" :min 200}])
...
(defn- positive-quantity [wine] (max 0 (or (:quantity wine) 0)))

(defn wine-quantity [wine] (positive-quantity wine))

(defn historical-quantity
  [wine]
  (max 0 (or (:original_quantity wine) (:quantity wine) 0)))

(defn- in-stock? [wine] (pos? (positive-quantity wine)))

(defn- parse-year
  [value]
  (cond (number? value) (int value)
        (string? value) (let [trimmed (str/trim value)]
...
(defn- canonical-style
  ...
  (let [value (normalize-string style)]
    (cond (str/blank? value) "Unspecified"
  ...
  (->> entries
       (reduce (fn [acc {:keys [label wines bottles]}]
                 (let [clean-label (if (str/blank? label) "Unspecified" label)
                       entry (get acc
                                  clean-label
                                  {:label clean-label :wines 0 :bottles 0})
               ...
               {})
       vals
       (sort-by (fn [{:keys [bottles wines]}] [(- bottles) (- wines)]))
       vec))
...
(defn- aggregate-by
  ...
  (aggregate-custom
   (map (fn [wine]
          {:label (label-fn wine) :wines 1 :bottles (positive-quantity wine)})
        wines)))
                ...
                (let [other-wines (reduce + 0 (map :wines remainder))
                  ...
                  (when (pos? other-bottles)
                    {:label other-label
                     ...
                     :bottle-share share})))]
    {:top top-with-share :other other}))

(defn- summarize-countries
  [wines total-bottles]
  (top-n-with-other (aggregate-by wines #(normalize-string (:country %)))
                    total-bottles
                    3
                    "Other"))
...
(defn- summarize-styles
  [wines total-bottles]
  (top-n-with-other (aggregate-by wines #(canonical-style (:style %)))
                    total-bottles
                    3
                    "Other"))
...
(defn- summarize-regions
  [wines total-bottles]
  (top-n-with-other (aggregate-by wines #(normalize-string (:region %)))
                    total-bottles
                    3
                    "Other"))
...
(defn- summarize-vintages
  ...
  (let [{:keys [entries non-vintage]}
        (reduce (fn [{:keys [entries non-vintage]} wine]
                  (let [qty (positive-quantity wine)
                        band (some-> (:vintage wine)
                                     parse-year
                                     band-label-for-year)]
                    (if band
                      {:entries (conj entries
                                      {:label band :wines 1 :bottles qty})
                       :non-vintage non-vintage}
                      {:entries entries :non-vintage (+ non-vintage qty)})))
        ...
        non-vintage-entry (when (pos? non-vintage)
                            {:label "Non-vintage"
                             ...
                             :bottle-share (when (pos? total-bottles)
                                             (/ non-vintage
                                                (double total-bottles)))})]
    {:bands band-entries :non-vintage non-vintage-entry}))
...
(defn- summarize-price-bands
  [wines total-bottles]
  (top-n-with-other (aggregate-by wines #(price->band-label (:price %)))
                    total-bottles
                    3
                    "Other price bands"))
...
(defn- normalized-window
  ...
  (let [from (parse-year (:drink_from_year wine))
    ...
    (cond (and from to) {:from (min from to) :to (max from to)}
...
(defn- in-window?
  [year {:keys [from to]}]
  (and (or (nil? from) (<= from year)) (or (nil? to) (>= to year))))
...
(defn- drinking-window-status
  ...
  (let [year (current-year)
    ...
    (cond (and to-num (> year to-num)) "Past window"
  ...
  (let [year (current-year)
    ...
    (when (pos? ready-bottles)
      {:year year
       ...
       :styles (add-share (aggregate-by ready-wines
                                        #(canonical-style (:style %)))
                          ready-bottles)})))

(defn total-bottles
  ([wines] (total-bottles wines wine-quantity))
  ([wines quantity-fn] (reduce + 0 (map quantity-fn wines))))

(defn total-value
  ([wines] (total-value wines wine-quantity))
  ([wines quantity-fn]
   (reduce (fn [acc wine]
...
(defn- normalized-label
  [value fallback]
  (let [label (normalize-string value)] (if (str/blank? label) fallback label)))
...
(defn- series-key
  ...
  (let [prefix (name (or group-by :overall))
        base (-> (str label)
                 ...
                 (str/replace #"[^a-z0-9]+" "-")
                 (str/replace #"^-+|-+$" ""))]
    (if (str/blank? base) prefix (str prefix "-" base))))
...
(defn- tally-unscheduled
  ...
  (reduce (fn [{:keys [wines bottles] :as acc} {:keys [window quantity]}]
            (if window acc {:wines (inc wines) :bottles (+ bottles quantity)}))
          {:wines 0 :bottles 0}
          entries))

(defn- ensure-overall-group
  [grouped entries add-overall?]
  (if add-overall? (assoc grouped "All wines" entries) grouped))
...
(defn- points-for-year
  ...
  (let [matching (filter (fn [{:keys [window]}]
    ...
    {:year year :wines (count matching) :bottles total-bottles}))
   ...
   (let [entries (map #(window-entry grouping %) wines)
     ...
     (if (seq windows)
       (let [min-year (apply min (map (comp :from :window) windows))
             ...
             grouped (ensure-overall-group (clojure.core/group-by :label
                                                                  windows)
                                           windows
                                           (some? grouping))
...
(defn collection-stats
  ...
  ([wines {:keys [visible-wines]}]
   (let [all-wines (vec (or wines []))
         ...
         subsets {:all all-wines :in-stock in-stock-wines :selected visible}
         counts (into {}
                      (map (fn [[subset coll]] [subset (count coll)]) subsets))
         bottle-totals (into {}
                             (map (fn [[subset coll]]
                                    (let [quantity-fn (if (= subset :all)
                                                        ...
                                                        wine-quantity)]
                                      [subset
                                       (total-bottles coll quantity-fn)]))
                                  subsets))
         rating-totals
         (into {}
               (map (fn [[subset coll]] [subset (average-internal-rating coll)])
                    subsets))
         value-totals
         (into {}
               (map (fn [[subset coll]]
