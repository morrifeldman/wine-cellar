(ns wine-cellar.views.components.stats-charts
  (:require [reagent.core :as r]
            ["recharts" :refer [ResponsiveContainer AreaChart Area LineChart Line XAxis YAxis CartesianGrid Tooltip ReferenceLine Legend]]))

(def palette
  ["#64b5f6" "#ffb74d" "#ce93d8" "#81c784" "#4db6ac" "#f06292" "#ba68c8" "#7986cb" "#a1887f"])
...
(defn optimal-window-chart
  ...
  (let [metric (or metric :wines)
    ...
    (r/with-let [mounted? (r/atom false)]
           ...
           (if multi-series?
             [:> LineChart {:data (clj->js chart-data)
              ...
              [:> YAxis {:allowDecimals false
                         ...
                         :width 50
                         :stroke "rgba(255,255,255,0.6)"}]
              [:> Tooltip {:formatter (fn [value name props]
                                        (let [label (some-> props (.-payload) (.-year))]
                                          #js[(metric->label metric value) (str name " • " label)]))
                           :cursor (clj->js {:stroke "rgba(255,255,255,0.35)"})}]
              [:> Legend {:wrapperStyle (clj->js {:color "rgba(255,255,255,0.72)"})}]
              (when current-year
                [:> ReferenceLine {:x current-year
                                   :stroke "#ffffff"
                                   :strokeDasharray "3 3"
              ...
              (doall
               (map-indexed
                ...
                series))]
             [:> AreaChart {:data (clj->js chart-data)
                            :margin (clj->js {:top 20 :right 30 :bottom 0 :left 0})}
              [:> CartesianGrid {:strokeDasharray "3 3" :stroke "rgba(255,255,255,0.12)"}]
              [:> XAxis {:dataKey "year"
                         :tickFormatter (fn [value] (str value))
              ...
              [:> YAxis {:allowDecimals false
                         ...
                         :width 50
                         :stroke "rgba(255,255,255,0.6)"}]
              [:> Tooltip {:formatter (fn [value _ props]
