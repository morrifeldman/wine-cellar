(ns wine-cellar.views.wines.varieties
  (:require [reagent.core :as r]
            ...
            [wine-cellar.views.components.form :refer [form-container form-actions form-row select-field number-field]]))

(defn grape-variety->option [gv]
  {"id" (:id gv) "label" (:name gv)})
...
(defn wine-variety-form
  ...
  (let [editing-id (:editing-wine-variety-id @app-state)
        ...
        ;; Filter out varieties that are already added (unless we're editing)
        available-varieties (if editing-id
                              grape-varieties
                              (filter #(not (contains? added-variety-ids (:id %))) 
                                      grape-varieties))
        ...
        submit-handler (fn []
                         (if (nil? (:variety_id variety))
                           ...
                           (do
                             (swap! app-state assoc :submitting-wine-variety? true)
                             (if editing-id
                               (api/update-wine-variety-percentage 
                                app-state wine-id editing-id (:percentage variety))
                               (api/add-variety-to-wine app-state wine-id variety)))))]
    [form-container {:title (if editing-id "Edit Grape Variety" "Add Grape Variety")
     ...
     [form-row
      (if editing-id
        ...
        [select-field
         {:label "Grape Variety"
          :required true
          :value (when-let [gv (first (filter #(= (:id %) (:variety_id variety)) grape-varieties))]
                   (clj->js (grape-variety->option gv)))
          :options (clj->js (mapv grape-variety->option available-varieties))
          :on-change #(swap! app-state assoc-in [:new-wine-variety :variety_id] (.-id %))
          :helper-text (when (empty? available-varieties) 
      ...
      [number-field
       {:label "Percentage"
        ...
        :on-change #(swap! app-state assoc-in 
                      [(if editing-id :editing-wine-variety :new-wine-variety) :percentage] 
     ...
     [dialog-content
      [typography
       (str "Are you sure you want to remove the grape variety '"
            (:variety_name grape-variety) "' from this wine?")]]
     [dialog-actions
            ...
            [grid {:item true :xs 2 :textAlign "right"}
             [icon-button {:edge "end"
                           ...
                           :on-click #(swap! app-state assoc 
                                       ...
                                       :show-wine-variety-form? true)}
              [edit]]
             [icon-button {:edge "end"
                           :aria-label "delete"
                           :on-click #(swap! app-state assoc 
                                       :deleting-wine-variety-id (:variety_id variety))}
              [delete]]]]])])
...
(defn wine-varieties-component
  ...
  (r/create-class
   {:component-did-mount
    ...
    :reagent-render
    (fn [app-state wine-id]
      [wine-varieties-list app-state wine-id])}))
