(ns wine-cellar.views.wines.form
  (:require [clojure.string :as str]
            ...
            [wine-cellar.views.components.barcode-scanner :refer
             [barcode-scanner]]))
  ...
  (let [new-wine (:new-wine @app-state)
    ...
    [form-container {:title "Add New Wine" :on-submit submit-handler}
     ...
     [form-row
      ...
      [barcode-scanner
       {:value (:barcode new-wine)
        :on-change #(swap! app-state assoc-in [:new-wine :barcode] %)
        :on-scan #(swap! app-state assoc-in [:new-wine :barcode] %)
        :label "Wine Barcode"}]]
     [form-row
      [currency-field
       {:label "Price"
