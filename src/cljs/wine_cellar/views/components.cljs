(ns wine-cellar.views.components
  (:require [clojure.string :as str]
            ...
            [wine-cellar.utils.vintage :refer [tasting-window-status tasting-window-color]]
...
(defn wine-classification-info [wine]
  [grid {:item true :xs 6}
   [box {:sx {:display "flex"
              :alignItems "center"}}
    [typography {:variant "body2"
...
(defn wine-details-grid [wine]
  [grid {:container true :spacing 0.5}  ;; Reduced spacing
   ...
   [wine-location-info wine]
   [wine-price-info wine]])

(defn wine-rating-display [wine]
  [box {:sx {:display "flex"
             :alignItems "center"}}
...
(defn wine-tasting-window [status drink-from-year drink-until-year]
  [box {:sx {:display "flex"
             :alignItems "center"}}
...
(defn wine-quantity-display [app-state wine]
  [box {:sx {:display "flex"
             :alignItems "center"}}
   [typography {:variant "body2"
                :color "text.secondary"
...
(defn wine-action-buttons [app-state wine]
  [box {:sx {:display "flex"
             :gap 1}}
   [button
    {:variant "outlined"
...
(defn wine-controls [app-state wine]
  [box {:sx {:display "flex"
             :justifyContent "space-between"
             :alignItems "center"
             :mt 1}}  ;; Reduced margin
  ...
  (let [status (tasting-window-status wine)
    ...
    [paper {:elevation 2
            :sx {:p 1.5  ;; Reduced padding
                 ...
                 :backgroundImage (when (= (:style wine) "Red")
