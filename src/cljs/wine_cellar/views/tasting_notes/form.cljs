(ns wine-cellar.views.tasting-notes.form
  (:require [wine-cellar.views.components.form
...
(defn tasting-note-form [app-state wine-id]
  (let [wine (first (filter #(= (:id %) wine-id)
                            (:wines @app-state)))
        current-drink-from (:drink_from_year wine)
        current-drink-until (:drink_until_year wine)
        new-note (:new-tasting-note @app-state)]

    ;; Initialize or update the tasting window fields with current values from the wine
    ;; We need to update in these cases:
    ;; 1. First time initialization (fields are nil)
    ;; 2. Switching to a different wine
    ;; 3. Wine values have changed (always update to reflect external changes)
    (when (or (nil? (:drink_from_year new-note))
      ...
      (swap! app-state update :new-tasting-note
             #(-> %
       ...
       {:title "Add Tasting Note"
        :on-submit #(do
                      ...
                      (when (or (not= (:drink_from_year updated-note) current-drink-from)
                                (not= (:drink_until_year updated-note) current-drink-until))
                        (api/update-wine
                         app-state
                         wine-id
                         (select-keys
       ...
       [form-row
        [year-field
         {:label "Drink From Year"
          ...
          :on-change #(swap! app-state update :new-tasting-note
                             (fn [note]
                               (-> note
                                   (assoc :drink_from_year (when-not (empty? %) (js/parseInt % 10)))
        ...
        [year-field
         {:label "Drink Until Year"
          ...
          :on-change #(swap! app-state update :new-tasting-note
                             (fn [note]
                               (-> note
                                   (assoc :drink_until_year (when-not (empty? %) (js/parseInt % 10)))
                                   (assoc :form-edited true))))}]]
