...
(defn handle-api-response [response error-msg]
  (cond
    ...
    (= 401 (:status response))
    (do
      (js/console.log "Authentication required, redirecting to login")
      ;; Don't try to parse the response body if it's not valid JSON
      (js/setTimeout #(set! (.-location js/window) (str api-base-url "/auth/google")) 100)
...
(defn api-request [method url params error-msg]
...
(defn GET [url error-msg]
  (api-request http/get url nil error-msg))

(defn POST [url params error-msg]
  (api-request http/post url params error-msg))

(defn PUT [url params error-msg]
  (api-request http/put url params error-msg))

(defn DELETE [url error-msg]
  (api-request http/delete url nil error-msg))
...
(defn fetch-classifications [app-state]
  (go
    (let [result (<! (GET "/api/classifications" "Failed to fetch classifications"))]
      (if (:success result)
...
(defn fetch-regions [app-state country]
  (go
    (let [result (<! (GET (str "/api/classifications/regions/" country)
...
(defn fetch-wines [app-state]
  ...
  (go
    (let [result (<! (GET "/api/wines" "Failed to fetch wines"))]
      (if (:success result)
        (do
...
(defn fetch-tasting-notes [app-state wine-id]
  (go
    (let [result (<! (GET (str "/api/wines/" wine-id "/tasting-notes")
...
(defn create-tasting-note [app-state wine-id note]
  (go
    (let [result (<! (POST (str "/api/wines/" wine-id "/tasting-notes")
                       note "Failed to create tasting note"))]
      (if (:success result)
        (do
...
(defn update-tasting-note [app-state wine-id note-id note]
  (go
    (let [result (<! (PUT (str "/api/wines/" wine-id "/tasting-notes/" note-id)
                       note "Failed to update tasting note"))]
      (if (:success result)
...
(defn delete-tasting-note [app-state wine-id note-id]
  (go
    (let [result (<! (DELETE (str "/api/wines/" wine-id "/tasting-notes/" note-id)
...
(defn adjust-wine-quantity [app-state wine-id adjustment]
  (go
    (let [result (<! (POST (str "/api/wines/" wine-id "/adjust-quantity")
...
(defn create-classification [app-state classification]
  ...
  (go
    (let [result (<! (POST "/api/classifications"
                       classification
