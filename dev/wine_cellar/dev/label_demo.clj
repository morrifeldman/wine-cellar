...
(defn- absolute-path [path] (.getAbsolutePath (io/file path)))
...
(defn- ensure-file!
  ...
  (let [f (io/file path)]
    (when-not (.exists f)
      (throw
       (ex-info
        (str "Sample image not found: " path)
        {:path path
         :hint
         (str
          "Drop the JPEG from the issue (e.g. Klimt Grüner front/back) into "
...
(defn- mime-type
  ...
  (let [lower (-> path
                  str/lower-case)]
    (cond (str/ends-with? lower ".png") "image/png"
          (or (str/ends-with? lower ".jpg") (str/ends-with? lower ".jpeg"))
          "image/jpeg"
...
(defn file->data-uri
  ...
  (let [file (ensure-file! path)
        bytes (Files/readAllBytes (Paths/get (.getAbsolutePath file)
                                             (make-array String 0)))
...
(defn run-all!
  ...
  ([front back]
   {:anthropic (run-anthropic! front back) :openai (run-openai! front back)}))

(comment
  ;; Evaluate forms in this block from a connected REPL to try the
  ;; workflow.
  ;; 1. Place the supplied Klimt Grüner JPGs into
  ;; dev/assets/klimt-front.jpg and .../klimt-back.jpg
  ;; 2. Require this namespace: (require
  ;; 'wine-cellar.dev.anthropic-label-demo)
