# Sensor Reading Telemetry API

This endpoint pair lets hardware (ESP32/Raspberry Pi, etc.) push environment readings directly into the wine-cellar backend and lets the UI fetch the recent history.
...
## Record a Reading
`POST /api/sensor-readings` (auth required)
...
Example using `curl`:
```bash
curl -X POST https://your-domain.example/api/sensor-readings \
...
`GET /api/sensor-readings?device_id=esp32-sentinel-1&limit=20`
...
curl -H "Authorization: Bearer $DEVICE_JWT" \
     "https://your-domain.example/api/sensor-readings?device_id=esp32-sentinel-1&limit=5"
```
...
`GET /api/sensor-readings/latest?device_id=esp32-sentinel-1`
...
`GET /api/sensor-readings/series?device_id=esp32-sentinel-1&bucket=1d&from=2025-11-01T00:00:00Z`
...
Each bucket returns `device_id`, `bucket_start` (ISO string), and avg/min/max for temperature, humidity, pressure, CO2, and battery voltage. Use this for long-range charts without pulling millions of raw samples.
